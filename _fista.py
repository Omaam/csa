...
def fista(infile1, infile2, freqinfo, lam):
    # load infile
    ...
    # load frequency setup file
    freq_lo = freqinfo[0]
    freq_up = freqinfo[1]
    nfreq   = freqinfo[2]
    delta_freq = (freq_up - freq_lo) / nfreq

    freq_edge = np.linspace(freq_lo, freq_up, int(nfreq + 1))
    freqdata = (freq_edge[:-1] + freq_edge[1:]) / 2


    # make matrix file
    A_mat = mkmat_cs(data1[:,0], data2[:,0], freqdata)
    A_w_mat = mkmat_cs_w(data1[:,0], data2[:,0], freqdata)
    print('A_mat: {}'.format(A_mat.shape))
    print('A_w_mat: {}'.format(A_w_mat.shape))
