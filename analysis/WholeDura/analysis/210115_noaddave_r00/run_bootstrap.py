...
def main():
    ...
    if CCF:
        ...
        # figure
        fig, ax = plt.subplots(3, figsize=(5, 7), sharex=True)

        # observed (FIRFIR) and XPS + OPS
        lags, corr = ccf(zscore(data1[:, 1]), zscore(data2[:, 1]),
                         fs=Fs, maxlags=10)
        ax[0].plot(lags, corr, color='grey')
        ax[0].plot(lags, q_med_res, color='cyan')
        ax[0].legend(['original (filterd)', 'residual'])
        ax[0].set_ylabel('r')

        # XOPS
        ax[1].plot(lags, q_med_xops, color='r')
        ax[1].fill_between(lags, q_hig_xops, q_low_xops, alpha=.5, color='r')
        ax[1].set_ylabel('r')

        # XPS and OPS
        ax[2].fill_between(lags, q_hig_xps, q_low_xps, alpha=.5)
        ax[2].fill_between(lags, q_hig_ops, q_low_ops, alpha=.5)
        ax[2].plot(lags, q_med_xps, lags, q_med_ops)
        ax[2].legend(['xps', 'ops'])
        ax[2].set_xlabel('lag')
        ax[2].set_ylabel('r')
