...
if __name__ == '__main__':
    ...
    lcname_list = [
                   ['./gx339_x_original.dat', './gx339_o_original.dat'],
                   ['./FIR2Hz/gx339_x_fir_original.dat', './FIR2Hz/gx339_o_fir_original.dat'],
                  ]
    texts = ['(a)', '(b)', '(c)', '(d)']
    for i, lcnames in enumerate(lcname_list):
        lcdata = np.loadtxt(lcnames[0])
        wave = lcdata[:,1]
        f, t, Zxx = signal.stft(wave, fs=20, nperseg=1000, noverlap=900)
        print(np.abs(Zxx.min()))
        print(np.abs(Zxx.max()))
        ax[i,0].pcolormesh(t, f, np.log10(np.abs(Zxx)), cmap='inferno')
        # ax[i,0].pcolormesh(t, f, (np.abs(Zxx))**(1/4), cmap='inferno')
        ...
        print(np.abs(Zxx.min()))
        print(np.abs(Zxx.max()))
        ax[i,1].pcolormesh(t, f, np.log10(np.abs(Zxx)), cmap='inferno')
        # ax[i,1].pcolormesh(t, f, (np.abs(Zxx))**(1/4), cmap='inferno')
    ...
    # save
    print('save fig')
    fig.savefig('spectrogram_201015.png', transparent=True, dpi=300)
