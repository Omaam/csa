...
if __name__ == '__main__':
    ...
    lcname_list = [
                   ...
                   ['./data/gx339_x_ma1s.dat', './data/gx339_o_ma1s.dat'],
                  ]
    texts = ['(a)', '(b)', '(c)', '(d)']
    for i, lcnames in enumerate(lcname_list):
        lcdata = np.loadtxt(lcnames[0])
        wave = lcdata[:,1]
        f, t, Zxx = signal.stft(wave, fs=20, nperseg=1000, noverlap=900, window='boxcar')
        ...
        wave = lcdata[:,1]
        f, t, Zxx = signal.stft(wave, fs=20, nperseg=1000, noverlap=990, window='boxcar')
    ...
    # save
    print('save fig')
    fig.savefig('spectrogram_rec.png', transparent=True, dpi=300)
