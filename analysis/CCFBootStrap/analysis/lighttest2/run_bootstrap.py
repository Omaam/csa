...
if os.path.exists('log') is False:
    os.mkdir('log')
logger = logging.getLogger(os.path.join('log', __name__))
...
def main():
    ...
    if STCS:
        ...
        for _ in tqdm(range(NBOOT)):
            freqs, t, X = stcs(data1, data2, FREQINFO, lam_min,
                               ...
                               max_workers=CORE, set_verbose=SET_VERBOSE)
    ...
    if ISTCS:
        ...
        logger.info('start boot iter for istcs')
        for i in tqdm(files_X_num):
            ...
            data1_rec, data2_rec = istcs(X, data1, data2, FREQINFO,
                                         TPERSEG, TOVERLAP, used_win='hann',
                                         max_workers=CORE,
                                         set_verbose=SET_VERBOSE,
        ...
        logger.info('end boot iter for istcs')
    ...
    if CCF:
        ...
        lags, q_low_all, q_med_all, q_hig_all = ccfbootstrap(
            Y1_all, Y2_all, DROPRATE, maxlags=10, fs=Fs)
        ...
        lags, q_low_xps, q_med_xps, q_hig_xps = ccfbootstrap(
            Y1_xps, Y2_xps, DROPRATE, maxlags=10, fs=Fs)
        ...
        lags, q_low_ops, q_med_ops, q_hig_ops = ccfbootstrap(
            Y1_ops, Y2_ops, DROPRATE, maxlags=10, fs=Fs)
        ...
        lags, q_low_xops, q_med_xops, q_hig_xops = ccfbootstrap(
            Y1_xops, Y2_xops, DROPRATE, maxlags=10, fs=Fs)
