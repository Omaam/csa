...
def main():
    ...
    data1, data2 = preprocess_for_gx339(
        '../../data/gx339_night1_2.txt', ndata=1500)
    ...
    # figure
    fig, ax = plt.subplots(2, 3, figsize=(7, 5), sharex=True)

    # observed (FIRFIR) and XPS + OPS
    ax[0, 0].plot(data1[:, 0], data1[:, 1], color='grey')
    # ax[0, 0].fill_between(data1[:, 0],
    #                       y1_low_res[:],
    #                       y1_hig_res[:],
    #                       y1_med_res[:], color='cyan')
    ax[0, 0].legend(['original (filterd)', 'residual'])
    ax[0, 0].set_ylabel('flux')
    ax[1, 0].plot(data2[:, 0], data2[:, 1], color='grey')
    # ax[1, 0].fill_between(data2[:, 0],
    #                       y2_low_res[:],
    #                       y2_hig_res[:],
    #                       y1_med_res[:], color='cyan')
    ax[1, 0].legend(['original (filterd)', 'residual'])
    ax[1, 0].set_ylabel('flux')

    # XOPS
    ax[0, 1].fill_between(data1[:, 0],
                          y1_low_xops[:],
                          y1_hig_xops[:],
                          y1_med_xops[:], color='r')
    ax[0, 1].legend(['xops'])
    ax[0, 1].set_ylabel('flux')
    ax[1, 1].fill_between(data2[:, 0],
                          y2_low_xops[:],
                          y2_hig_xops[:],
                          y1_med_xops[:], color='r')
    ax[1, 1].legend(['xops'])
    ...
    # XPS and OPS
    ax[0, 2].fill_between(data1[:, 0],
                          y1_low_xps[:],
                          y1_hig_xps[:],
                          y1_med_xps[:], color='tab:orange')
    ax[0, 2].fill_between(data1[:, 0],
                          y1_low_ops[:],
                          y1_hig_ops[:],
                          y1_med_ops[:], color='tab:blue')
    ax[0, 2].legend(['xps', 'ops'])
    ax[0, 2].set_ylabel('flux')
    ax[1, 2].fill_between(data2[:, 0],
                          y2_low_xps[:],
                          y2_hig_xps[:],
                          y2_med_xps[:], color='tab:orange')
    ax[1, 2].fill_between(data1[:, 0],
                          y2_low_ops[:],
                          y2_hig_ops[:],
                          y2_med_ops[:], color='tab:blue')
    ax[1, 2].legend(['xps', 'ops'])
    ax[1, 2].set_ylabel('flux')
